...
### Step 4: Replicate Merging, Annotation, and Differential Analysis

#### Replicate / group merging (`p1_MergeSite.py`)

  - Requires sites to pass depth + ratio thresholds in ≥2 replicates
  - Uses:
    - `cutoff.filterdep` (upper value)
    - `cutoff.ratio`

#### Genome annotation

  - Annotate sites to rRNA, gene body, intron, transcript coordinates

#### Differential testing
  - MethylKit-based testing
  - Produces:
    - `plot/DMA.{group}.genome.pseusite.txt`
    - clustering and PCA plots

#### Summary plots (`R3_PlotSummary.R`)
  
  - **Genome-only**
    - Applies:
    - `cutoff.alpha` (FDR / q-value)
    - `cutoff.diff` (effect size threshold)

## Cutoff Usage Summary

| Cutoff | Used in | Purpose |
|------|--------|--------|
| `mindep` | rRNA QC | QC visualization |
| `filterdep` | p0 / p1 scripts | Site depth filtering |
| `ratio` | p0 / p1 scripts | Treated enrichment |
| `alpha` | R3 plots | Statistical significance |
| `diff` | R3 plots | Effect size filtering |
