...
My advisor flagged that a number of sites reach implausibly small p-values
(`-log10(p) > 100`) given the small sample size (50 cell lines) and generally
modest expected QTL effect sizes, and suggested picking a few top hits and
plotting %pU by genotype directly to build intuition for what's driving them.
...
rows <- rbindlist(lapply(seq_len(nrow(top20)), function(i) {
  ...
  data.table(
    ...
    site_lab = sprintf("#%d chr%s:%.0f\n-log10(p)=%.0f%s",
                        s$rank, s$ref, s$pos, -log10(max(s$nominal_min_p, 1e-300)),
                        if (s$lead_pos == s$pos) "\n[SNP-at-site]" else ""),
...
ggplot(rows, aes(x = g, y = pi, color = g)) +
  ...
  labs(x = "genotype (copies of alt allele) at the lead cis-SNP", y = "%pU (y/n per cell line)",
       ...
       subtitle = "Ranked by nominal (pre-permutation) p-value, most extreme first. '[SNP-at-site]' = lead SNP sits exactly at the profiled base.") +
...
extreme "hits" in this model. Visually, nearly every panel above shows the
same suspiciously clean pattern: genotype 0 near %pU=1, genotype 1 near
%pU=0.5, genotype 2 near %pU=0, with almost no within-group scatter — not the
noisier, more modest dose-response shape expected of a real cis-regulatory
...
none of these are on chr6) show the *exact same* suspiciously clean staircase
