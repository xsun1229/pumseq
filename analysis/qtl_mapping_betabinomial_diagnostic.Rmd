...
rows <- rbindlist(lapply(seq_len(nrow(top20)), function(i) {
  ...
  data.table(
    ...
    g = factor(g, levels = 0:2), pi = pi_v, n = n, y = y,
...
# depth-weighted mean per (site, genotype group) -- sum(y)/sum(n), the exact
# quantity a binomial/beta-binomial GLM fits as that group's mean (verified
# directly against glm() fitted values in the conversation behind this page)
dw_means <- rows[, .(dw_mean = sum(y) / sum(n)), by = .(site_lab, g)]
...
ggplot(rows, aes(x = g, y = pi, color = g)) +
  ...
  labs(x = "genotype (copies of alt allele) at the lead cis-SNP", y = "%pU (y/n per cell line)",
       ...
       subtitle = "Black diamonds = depth-weighted group mean (sum(y)/sum(n), what the GLM actually fits). '[SNP-at-site]' = lead SNP at the profiled base; '[small group]' = smallest genotype class <5 cell lines; '[depth-dominated]' = one cell line >50% of its group's total depth.") +
...
dw_means_p1 <- rows_p1[, .(dw_mean = sum(y) / sum(n)), by = .(site_lab, g)]
...
ggplot(rows_p1, aes(x = g, y = pi, color = g)) +
  ...
  labs(x = "genotype (fixed) vs. PERMUTED phenotype at the lead cis-SNP",
       ...
       subtitle = "Black diamonds = depth-weighted group mean (sum(y)/sum(n), what the GLM actually fits). Same seed as the LRT calibration check's perm1 (set.seed(1001)).") +
  theme_bw(base_size = 11) +
  theme(legend.position = "bottom", strip.text = element_text(size = 7.5))
```
