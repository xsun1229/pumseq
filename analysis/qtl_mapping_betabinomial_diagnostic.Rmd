...
3. **Depth-dominated genotype groups** — the model weights each cell line by
   ...
   permutation, **0** have the same lead SNP — the specific SNP "blamed" is
...
specific real association. These sites also have very high median read
...
problem (`lambda`) from the permutation calibration check. We reran the same
5-seed LRT calibration check (`5.calibration_check_lrt.R`), adding a
`--exclude-site-snp` flag that drops any cis-SNP sitting exactly on a site's
